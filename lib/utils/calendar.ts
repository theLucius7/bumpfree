...
type CalendarSchedule = Pick<
  Schedule,
  "id" | "semester_tag" | "start_date" | "max_weeks"
> & { timezone?: string };
...
export function expandCourses(
  ...
  if (
    !week1Monday ||
    week1Monday.getDay() !== 1 ||
    !Number.isInteger(schedule.max_weeks) ||
    schedule.max_weeks < 1 ||
    schedule.max_weeks > MAX_CALENDAR_WEEKS
  )
    return events;
  ...
  for (const course of courses.slice(0, MAX_CALENDAR_COURSES)) {
    if (
      ...
      course.name.trim().length < 1 ||
      course.name.length > 200 ||
      (course.room !== null &&
        (typeof course.room !== "string" || course.room.length > 120)) ||
      (course.teacher !== null &&
        (typeof course.teacher !== "string" || course.teacher.length > 120)) ||
      !Number.isInteger(course.day_of_week) ||
      course.day_of_week < 1 ||
      course.day_of_week > 7 ||
      !Number.isInteger(course.start_week) ||
      !Number.isInteger(course.end_week) ||
      course.start_week < 1 ||
      course.end_week < course.start_week ||
      !CLOCK_PATTERN.test(course.start_time) ||
      !CLOCK_PATTERN.test(course.end_time) ||
    ...
    for (let week = firstWeek; week <= lastWeek; week++) {
      const courseDay = addDays(
        addWeeks(week1Monday, week - 1),
        course.day_of_week - 1,
      );
      const start = fromZonedTime(
        format(courseDay, "yyyy-MM-dd") + "T" + course.start_time,
        schedule.timezone || "Asia/Shanghai",
      );
      const end = fromZonedTime(
        format(courseDay, "yyyy-MM-dd") + "T" + course.end_time,
        schedule.timezone || "Asia/Shanghai",
      );
...
export function expandBusyBlocks(
  blocks: BusyBlock[],
  userId: string,
  displayName: string,
  memberColor: string,
  ...
  for (const block of blocks.slice(0, MAX_CALENDAR_BUSY_BLOCKS)) {
    ...
    if (
      ...
      block.title.trim().length < 1 ||
      block.title.length > 160 ||
      (block.note !== null &&
        (typeof block.note !== "string" || block.note.length > 1_000)) ||
      !isBoundedDate(start) ||
      !isBoundedDate(end) ||
      end <= start ||
...
export function expandMalaysiaHolidays(
  ...
  for (const holiday of holidays.slice(0, 1_000)) {
    ...
    events.push({
      ...
      resource: {
        ...
        displayName: "MY",
        color: "#64748b",
        courseName: holiday.localName || holiday.name,
        room: null,
        teacher: null,
        note: holiday.name,
      },
...
export function getUsersOnDate(events: CalendarEvent[], date: Date): string[] {
  ...
  for (const ev of events)
    if (isBoundedDate(ev.start) && localDateKey(ev.start) === dateStr)
