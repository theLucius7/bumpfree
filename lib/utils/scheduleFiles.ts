...
export async function extractScheduleFileText(file: File): Promise<string> {
  ...
  if (extension === ".pdf" || file.type === "application/pdf") {
    throw new Error(
      "暂不支持直接解析 PDF；请先在本地或可信 AI 工具中转为 UTF-8 文本，再粘贴或上传 TXT",
    );
  } else if (
    extension === ".docx" ||
    file.type ===
...
function assertSafeZipContainer(buffer: Buffer) {
  ...
  for (let index = 0; index < entryCount; index += 1) {
    ...
    if (compressionMethod !== 0 && compressionMethod !== 8)
      throw new Error("压缩文件使用了不支持的压缩算法");
    ...
    if (
      declaredUncompressedSize > MAX_ZIP_ENTRY_SIZE ||
    ...
    if (
      actualUncompressedSize > MAX_ZIP_ENTRY_SIZE ||
      totalUncompressedSize > MAX_ZIP_TOTAL_SIZE
    ) {
      throw new Error("压缩文件解压后过大");
    }
...
function extractSpreadsheetText(buffer: Buffer): string {
  ...
  if (workbook.SheetNames.length === 0)
    throw new Error("Excel 工作簿没有可读取的工作表");
  if (workbook.SheetNames.length > MAX_WORKSHEETS)
    throw new Error(`Excel 工作表不能超过 ${MAX_WORKSHEETS} 个`);
  ...
  for (const sheetName of workbook.SheetNames) {
    ...
    if (cellCount > MAX_SPREADSHEET_CELLS) {
      throw new Error(
        `Excel 有效单元格范围不能超过 ${MAX_SPREADSHEET_CELLS.toLocaleString("en-US")} 格`,
      );
    }
