...
for (const sample of parserSamples) {
    ...
    assert.equal(parsed.courses.length, sample.expect.count, `${sample.label}: course count`);
    assert.equal(parsed.format, sample.expect.format, `${sample.label}: format`);
    if (sample.expect.firstName) assert.equal(parsed.courses[0]?.name, sample.expect.firstName, `${sample.label}: first course name`);
    if (sample.expect.firstRoom) assert.equal(parsed.courses[0]?.room, sample.expect.firstRoom, `${sample.label}: first room`);
    if (sample.expect.firstDay) assert.equal(parsed.courses[0]?.dayOfWeek, sample.expect.firstDay, `${sample.label}: first day`);
...
for (const sample of rejectedSamples) {
    assert.throws(
        ...
        (error: unknown) => error instanceof Error && error.message.includes(sample.message),
...
assert.throws(
    () => parseTextSchedule("周一10:00-12:00CourseA1#1011-2", { adapterKey: "unknown" }),
    /学校专用导入接口请通过 scheduleAdapterRegistry 调用/,
    "unknown adapter keys must be rejected",
);
console.log("PASS unknown adapter key: rejected");

const strictCourse = (startDate: string, maxWeeks: number, weeks: string) => `BumpFree Schedule Import v1
Semester: validation
StartDate: ${startDate}
Timezone: Asia/Shanghai
MaxWeeks: ${maxWeeks}
...
assert.throws(() => parseTextSchedule(strictCourse("2026-02-30", 14, "1-14")), /不是有效日期/);
assert.throws(() => parseTextSchedule(strictCourse("2026-04-07", 14, "1-14")), /周一/);
assert.throws(() => parseTextSchedule(strictCourse("2026-04-06", 31, "1-31")), /1-30/);
...
const databaseClockEvents = expandCourses([
    ...
    max_weeks: 1,
}, "00000000-0000-4000-8000-000000000003", "Test User", "#2563eb");
assert.equal(databaseClockEvents.length, 1, "PostgREST HH:MM:SS clock values must render");
assert.equal(databaseClockEvents[0]?.start.getHours(), 9);
assert.equal(databaseClockEvents[0]?.end.getMinutes(), 30);
console.log("PASS PostgREST database clock rendering");

const wakeUpPayload = [
    JSON.stringify({ version: 1 }),
    JSON.stringify([
        { node: 1, startTime: "08:00", endTime: "08:45", timeTable: 1 },
        { node: 2, startTime: "08:50", endTime: "09:35", timeTable: 1 },
    ]),
    JSON.stringify({
        startDate: "2026-4-6",
        maxWeek: 16,
        school: "Example University",
        tableName: "2026 Spring",
        showSat: false,
        showSun: false,
        sundayFirst: false,
    }),
    JSON.stringify([{ id: 7, courseName: "Algorithms", color: "#ff336699" }]),
    JSON.stringify([
        {
            id: 7,
            day: 2,
...
const wakeUpSchedule = parseWakeUpResponse(wakeUpPayload);
assert.deepEqual(wakeUpSchedule, {
    semesterTag: "2026 Spring",
    school: "Example University",
    startDate: "2026-04-06",
    maxWeeks: 16,
    courses: [
...
assert.equal(extractWakeUpKey("分享口令为「0123456789ABCDEF0123456789ABCDEF」"), "0123456789abcdef0123456789abcdef");
assert.equal(extractWakeUpKey("0123456789abcdef0123456789abcdef"), "0123456789abcdef0123456789abcdef");
assert.equal(extractWakeUpKey("0123456789ABCDEF0123456789ABCDEF"), "0123456789abcdef0123456789abcdef");
assert.equal(extractWakeUpKey("not a key"), null);
assert.throws(() => parseWakeUpResponse(wakeUpPayload.replace('"maxWeek":16', '"maxWeek":31')), /总周数/);
assert.throws(() => parseWakeUpResponse("x".repeat(500_001)), /数据大小/);
console.log("PASS WakeUp response and share-key parser");

async function testScheduleFileExtraction() {
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(
        workbook,
        XLSX.utils.aoa_to_sheet([["Day", "Course"], ["Monday", "Algorithms"]]),
        "Schedule",
    );
    const workbookBytes = XLSX.write(workbook, { bookType: "xlsx", type: "buffer" });
    const workbookText = await extractScheduleFileText(new File(
        [workbookBytes],
        "schedule.xlsx",
    ...
    await assert.rejects(
        () => extractScheduleFileText(new File(["not a workbook"], "fake.xlsx", { type: "application/vnd.ms-excel" })),
        /有效的 Excel/,
    );
    const forgedZip = createForgedSmallZip(20 * 1024 * 1024 + 1);
    await assert.rejects(
        () => extractScheduleFileText(new File(
            [Uint8Array.from(forgedZip)],
            "forged.docx",
            { type: "application/vnd.openxmlformats-officedocument.wordprocessingml.document" },
        )),
        /解压后过大/,
    );
    console.log("PASS bounded XLSX extraction, signatures and forged ZIP-size rejection");
}

function createForgedSmallZip(actualUncompressedSize: number): Buffer {
    const fileName = Buffer.from("word/document.xml", "utf8");
    const compressed = deflateRawSync(Buffer.alloc(actualUncompressedSize, 0x41), { level: 9 });
    const forgedSize = 1;
