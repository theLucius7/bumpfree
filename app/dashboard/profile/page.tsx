...
import { IcsImportPanel } from "@/components/dashboard/IcsImportPanel";
...
export default function ProfilePage() {
  const { data, error } =
    useResource<import("@/lib/api").SchedulesData>("data/schedules");
  if (!data) return <RequestState error={error} />;
  const { profile, schedules, importInterfaces } = data;
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            通过 ICS 日历文件导入课表数据，支持多课表存档管理
          </p>
        </div>
        <div className="text-right">
          <Badge
            variant={quotaReached ? "destructive" : "secondary"}
            className="text-xs"
          >
        ...
        <div className="p-4 rounded-xl border border-destructive/20 bg-destructive/5 text-sm text-destructive flex items-start gap-3">
          ...
          <div>
            ...
            <p className="opacity-90">
              你的账号目前最多保存 {quota}{" "}
              份课表。仍可覆盖或追加到同名学期；新建学期前请先删除下方的过期课表。
            </p>
          </div>
        </div>
      )}

      <IcsImportPanel canCreateSchedule={!quotaReached} />
      <details>
        ...
        <div className="mt-3">
          <ScheduleImportPanel
            ...
            canCreateSchedule={!quotaReached}
          />
        </div>
      </details>
                ...
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                      {schedule.school} · 起始 {schedule.start_date} ·{" "}
                      {schedule.max_weeks} 周
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    {!schedule.is_active && (
                      <form
                        action={async () => {
                          const result = await setActiveSchedule(schedule.id);
                    ...
                    (course: {
                      ...
                      >
                        <span
                          ...
                          style={{
                            backgroundColor: course.color ?? MEMBER_COLORS[0],
                          }}
                        />
                        <div className="min-w-0">
                          <p className="font-medium truncate">{course.name}</p>
                          <p className="text-muted-foreground text-xs">
                            {DAY_NAMES[course.day_of_week]} {course.start_time}-
                            {course.end_time}
      ...
      {!hasSchedule && (
        <div className="text-center py-12 text-muted-foreground">
          ...
          <p>还没有导入任何课表</p>
          <p className="text-sm mt-1">选择上方的 ICS 日历文件开始</p>
        </div>
      )}
    </div>
  );
