...
export default function DashboardPage() {
  const { data, error } =
    useResource<import("@/lib/api").DashboardData>("data/dashboard");
  if (!data) return <RequestState error={error} />;
  const { profile, roomCount, scheduleCount, invitationCount, memberRooms } =
    data;
  return (
...
function StatCard({
  ...
  return (
    <Link href={href}>
      <Card
        className={`hover:border-border transition-colors ${highlight ? "border-primary/50" : ""}`}
      >
        <CardContent className="pt-5 pb-4">
          <div
            className={`mb-2 ${highlight ? "text-primary" : "text-muted-foreground"}`}
          >
            {icon}
          </div>
