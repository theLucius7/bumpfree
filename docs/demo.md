...
已发布共享周历、ICS 导入预览和按人查看三张真实截图，均来自正式应用的实际操作，不修改 DOM、不伪造功能、不混入真实用户资料。公开日历截图展示 **2026-08-31 至 2026-09-06**。

ICS 导入预览使用仓库内的[虚构示例文件](../examples/demo-schedule.ics)，第 1 周周一设为 **2026-08-31**、学期设为 **20 周**，展示 5 门课程解析出的 12 条记录、72 次上课。截图仅停留在预览步骤，没有确认导入或修改公开 Demo 的现有课表，拍摄后已退出专用演示账号。社交预览中的向量示意图与这些真实使用截图分别标注。
